...
void Population::drawgraph(svg *svgObj) {
	int width = 1280; // SVGドキュメントの横幅
	int height = 400; // SVGドキュメントの縦幅
	int barHeight = height / values.size(); // 棒グラフの高さをデータの数に比例させる
	for(unsigned long i=0; i < values.size(); i++) {
		int barWidth = (values.at(i) * width) / 1500; // 棒グラフの幅をデータの値に比例させる
		svgObj->drawRect(100, i*barHeight+10, barWidth, barHeight-10); // 棒グラフを描画する
		svgObj->drawText(0, i*barHeight+25, labels.at(i)); // 棒グラフの上に項目名を描画する
	}
}